...
if __name__ == "__main__":
    ...
    list_subjects = DICOMTransit.orthanc.API.get_list_of_subjects_noauth(credential.url)
        ...
        try:
            ...
            if MRN_exist is False:  # most common scenario first
                ...
                DICOMTransit.LocalDB.API.set_CNBP(
                    DICOM_package.MRN, DICOM_package.CNBPID
                )
                DICOMTransit.LocalDB.API.set_DCCID(
                    DICOM_package.MRN, DICOM_package.DCCID
                )
                DICOMTransit.LocalDB.API.set_timepoint(
                    DICOM_package.MRN, DICOM_package.timepoint
                )
                DICOMTransit.LocalDB.API.set_scan_date(
                    DICOM_package.MRN, DICOM_package.scan_date
                )
                ...
                API_date_string = DICOMTransit.LocalDB.API.get_scan_date(
                    DICOM_package.MRN
                )
                ...
                DICOM_package.CNBPID = DICOMTransit.LocalDB.API.get_CNBP(
                    DICOM_package.MRN
                )

                # Use MRN to retrieve DCCID, update the dicom-package
                DICOM_package.DCCID = DICOMTransit.LocalDB.API.get_DCCID(
                    DICOM_package.MRN
                )
                ...
                latest_timepoint = DICOMTransit.LORIS.API.increment_timepoint(DCCID)
            ...
            # Anonymize to Zip
            DICOMTransit.DICOM.API.anonymize_to_zip(
                DICOM_package.dicom_folder, DICOM_package.zipname
            )

            # Upload
            DICOMTransit.LORIS.API.old_upload(
                os.path.join(
            ...
            # Trigger insertion.
            DICOMTransit.LORIS.API.new_trigger_insertion(DICOM_package.zipname)
        except:
