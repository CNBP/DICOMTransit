...
class UT_DevOrthanc(unittest.TestCase):
    ...
    def test_getStudies(self):
        ...
        subject_list = DICOMTransit.orthanc.API.get_all_subject_StudyUIDs(
            self.credential
        )
    ...
    def test_getSubjectZip(self):
        url, user, password = DICOMTransit.orthanc.API.get_prod_orthanc_credentials()
        list_subjects = self.test_getStudies()
        ...
        for subject in list_subjects:
            ...
            DICOMTransit.orthanc.API.get_StudyUID_zip(patient_file_url, self.credential)
