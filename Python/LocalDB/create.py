        ...
        try:
            ...
            c.execute(
                "CREATE TABLE {tn} ({nf} {ft} PRIMARY KEY)".format(
                    tn=TableName, nf=KeyFieldString, ft=KeyFieldType
                )
            )
                ...
                if (
                    ...
                    c.execute(
                        "ALTER TABLE {tn} ADD COLUMN '{cn}' {ct}".format(
                            tn=TableName, cn=column[0], ct=column[1]
                        )
                    )
            ...
            logger.info(e)
            logger.error(
                "SQLite database creation/update issue, suspect schema non-compliant SQLite database. Did you corrupt this SQLite database somehow?"
            )
            raise IOError
        return True
