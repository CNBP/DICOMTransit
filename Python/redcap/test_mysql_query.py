...
import unittest
from redcap.mysql_query import send_mysql_data, get_mysql_tablename, convert_redcap_to_mysql_fieldtype, convert_redcap_to_mysql_fieldname
...
class UT_REDCapMySQLQuery(unittest.TestCase):
    ...
    def test_get_mysql_tablename() -> None:
        ...
        # METADATA.
        metadata_test = [['admissioncasetype_descriptionfr', 'text', 'admissioncasetype_descriptionfr', 'test_metadata_tablename']]
        ...
        assert get_mysql_tablename(metadata_test, entries_test_redcap_repeat_instrument) == "test_redcap_repeat_instrument"

        # TEST #2 ( If no redcap_repeat_instrument, we need to use the complete field at the end. )
        assert get_mysql_tablename(metadata_test, entries_test_mstadmissioncasetype_complete) == "mstadmissioncasetype"

        # TEST #3 ( If no complete & no redcap_repeat_instrument is found then we need to match with the metadata list. )
        assert get_mysql_tablename(metadata_test, entries_test_metadata) == "test_metadata_tablename"

        # TEST #4 ( If no complete & no redcap_repeat_instrument is found then we need to match with the metadata list. )
        assert get_mysql_tablename(metadata_test, entries_test_metadata_unknown) == "unknown"
    ...
    def test_convert_redcap_to_mysql_fieldtype() -> None:
        ...
        # TEST - yes no field.
        assert convert_redcap_to_mysql_fieldtype(MySQLType.yesno.name) is MySQLType.yesno.value

        # TEST - text field.
        assert convert_redcap_to_mysql_fieldtype(MySQLType.text.name) is MySQLType.text.value

        # TEST - text primary field.
        assert convert_redcap_to_mysql_fieldtype(MySQLType.textprimary.name) is MySQLType.textprimary.value
    ...
    def test_convert_redcap_to_mysql_fieldname() -> None:
        ...
        assert convert_redcap_to_mysql_fieldname("msttablename","tablename_test") == "test"

        # TEST - Order to position convert.
        assert convert_redcap_to_mysql_fieldname("msttablename","tablename_ordertest") == "positiontest"

        # TEST - system to system_
        assert convert_redcap_to_mysql_fieldname("tablename","tablename_ordersystemtest") == "positionsystem_test"

        # TEST - Remove useless _
        assert convert_redcap_to_mysql_fieldname("tablename","tablename_test_test") == "test_test"
