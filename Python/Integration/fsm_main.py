...
def pacs():
    ...
    dispatcher.send(signal=SIG_INCOMING_DICOM, sender=current_funct_name())
...
def check_orthanc_srvr(signal, sender):
    ...
    try:
        ...
        dispatcher.send(signal=ERROR_Orthanc, sender=current_funct_name(), arglist=args)
...
def check_mrn_exists(signal=None, sender=None, DICOM_package: DICOMPackage = None):
    ...
    try:
        ...
        if MRN_exist:  # in the local SQLiteDB
            dispatcher.send(
                signal=SIG_GET_CNBPID_USING_MRN,
                sender=current_funct_name(),
                DICOM_package=DICOM_package,
            )
            dispatcher.send(
                signal=SIG_GET_DCCID_USING_MRN,
                sender=current_funct_name(),
                DICOM_package=DICOM_package,
            )
            dispatcher.send(
                signal=SIG_GET_VISIT_USING_MRN,
                sender=current_funct_name(),
                DICOM_package=DICOM_package,
            )
        else:  # not exist scenerio
            dispatcher.send(
                signal=SIG_GENERATE_CNBPID,
                sender=current_funct_name(),
                DICOM_package=DICOM_package,
            )
    except Exception as ex:
        ...
        args = {"Exception": ex, "locals": mylocals}
        dispatcher.send(signal=ERROR_SQLite, sender=current_funct_name(), arglist=args)
...
def postman(
    ...
    sender=None,
    from_signal=None,
    DICOM_package: DICOMPackage = None,
    arglist=None,
):
    print("Signal is", signal)
    ...
    if from_signal == SIG_GET_MRN_FROM_DICOM:
        if DICOM_package is not None:
            if DICOM_package.mrn is not None:
                dispatcher.send(
                    signal=SIG_GET_AND_ASSIGN_CNBPID_USING_MRN,
                    sender=postman,
                    DICOM_package=DICOM_package,
                )
    ...
    if from_signal == SIG_GET_AND_ASSIGN_CNBPID_USING_MRN:
        if DICOM_package is not None:
            if DICOM_package.cnbpid is not None:
                dispatcher.send(
                    signal=SIG_GET_LORISID_AND_VISIT,
                    sender=postman,
                    DICOM_package=DICOM_package,
                )
    ...
    if from_signal == SIG_ANONYMIZE_DATA:
        if DICOM_package is not None:
            if DICOM_package.is_anonymized is not None:
                dispatcher.send(
                    signal=SIG_UPLOAD_ANONYMIZED_DATA,
                    sender=postman,
...
dispatcher.connect(
    get_MRN_from_DICOM, signal=SIG_GET_MRN_FROM_DICOM, sender=dispatcher.Any
)
dispatcher.connect(check_mrn_exists, signal=SIG_CHECK_MRN_EXISTS, sender=dispatcher.Any)
dispatcher.connect(
    get_CNBPID_using_MRN, signal=SIG_GET_CNBPID_USING_MRN, sender=dispatcher.Any
)
dispatcher.connect(
    get_DCCID_using_MRN, signal=SIG_GET_DCCID_USING_MRN, sender=dispatcher.Any
)
dispatcher.connect(
    get_VISIT_using_MRN, signal=SIG_GET_VISIT_USING_MRN, sender=dispatcher.Any
)

dispatcher.connect(
    assign_cnbpid_using_mrn,
    signal=SIG_GET_AND_ASSIGN_CNBPID_USING_MRN,
...
dispatcher.connect(
    upload_anonymized_data, signal=SIG_UPLOAD_ANONYMIZED_DATA, sender=dispatcher.Any
)
